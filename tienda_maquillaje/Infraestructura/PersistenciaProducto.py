...
from tienda_maquillaje.Dominio import Producto


class PersistenciaProducto():

    def connect(self):
        self.con = sqlite3.connect("tienda_maquillaje.sqlite")
        self.__crear_tabla()
    ...
    def __crear_tabla(self):
        try:
            ...
            query = "CREATE TABLE PRODUCTO(id text primary key, nombre text," \
                " proveedor text, precio float) "
    ...
    def guardar_producto(self,producto : Producto):
        cursor = self.con.cursor()
        query = "insert into PRODUCTO(id , nombre ," \
                " proveedor , precio ) values(" \
                f" ?,?,?,?)"
        cursor.execute(query, (str(producto.id), producto.nombre, producto.proveedor,
                               producto.precio))
        self.con.commit()
